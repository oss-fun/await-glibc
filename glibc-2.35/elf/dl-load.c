...
static size_t is_dst(const char *input, const char *ref) {
	...
	if (is_curly) /* Count the two curly braces.  */
		return rlen + 2;
	else
		return rlen;
}
...
size_t _dl_dst_count(const char *input) {
	...
	if (__glibc_likely(input == NULL)) return 0;
		...
		if (__glibc_unlikely(*input == '$')) {
			...
			if ((len = is_dst(input, "ORIGIN")) != 0) {
				...
				if (__glibc_unlikely(__libc_enable_secure) &&
						!(input == start + 1 && (input[len] == '\0' || input[len] == '/')))
					repl = (const char *)-1;
				else
					repl = l->l_origin;
...
/* Return a malloc allocated copy of INPUT with all recognized DSTs
	 ...
	 is returned.  */
static char *expand_dynamic_string_token(struct link_map *l,
		const char *input) {
	...
	if (__glibc_likely(cnt == 0)) return __strdup(input);
...
static void add_name_to_object(struct link_map *l, const char *name) {
	...
	for (lnp = l->l_libname; lnp != NULL; lastp = lnp, lnp = lnp->next)
		if (strcmp(name, lnp->name) == 0) return;
...
static struct r_search_path_elem **fillin_rpath(
	...
	while ((cp = __strsep(&rpath, sep)) != NULL) {
		...
		if (*cp != '\0') {
			...
			if (len > 0 && cp[len - 1] != '/') cp[len++] = '/';
		}

		/* See if this directory is already known.  */
		for (dirp = GL(dl_all_dirs); dirp != NULL; dirp = dirp->next)
			if (dirp->dirnamelen == len && memcmp(cp, dirp->dirname, len) == 0) break;

		if (dirp != NULL) {
			/* It is available, see whether it's on our own list.  */
			size_t cnt;
			for (cnt = 0; cnt < nelems; ++cnt)
				if (result[cnt] == dirp) break;

			if (cnt == nelems) result[nelems++] = dirp;
		} else {
			size_t cnt;
			...
			dirp = (struct r_search_path_elem *)malloc(
					sizeof(*dirp) + ncapstr * sizeof(enum r_dir_status) + where_len +
					len + 1);
			if (dirp == NULL)
			...
			dirp->dirname =
				((char *)dirp + sizeof(*dirp) + ncapstr * sizeof(enum r_dir_status));
			*((char *)__mempcpy((char *)dirp->dirname, cp, len)) = '\0';
			dirp->dirnamelen = len;

			if (len > max_dirnamelen) max_dirnamelen = len;
			...
			for (cnt = 0; cnt < ncapstr; ++cnt) dirp->status[cnt] = init_val;
...
static bool cache_rpath(struct link_map *l, struct r_search_path_struct *sp,
	...
	if (sp->dirs == (void *)-1) return false;

	if (sp->dirs != NULL) return true;
...
void _dl_init_paths(const char *llp, const char *source,
	...
	aelem = __rtld_search_dirs.dirs = (struct r_search_path_elem **)malloc(
...
signal_error:
	...
	__rtld_search_dirs.dirs[0] = malloc(nsystem_dirs_len * round_size *
	...
	do {
		...
		*aelem++ = pelem;
		...
		for (cnt = 0; cnt < ncapstr; ++cnt) pelem->status[cnt] = unknown;
	...
	*aelem = NULL;
	...
	if (llp != NULL && *llp != '\0') {
		...
		__rtld_env_path_list.dirs = (struct r_search_path_elem **)malloc(
		...
		(void)fillin_rpath(llp_tmp, __rtld_env_path_list.dirs, ":;", source, NULL,
				l);
...
void _dl_process_pt_gnu_property(struct link_map *l, int fd,
	...
	if (align != (__ELF_NATIVE_CLASS / 8)) return;
	...
	while ((ElfW(Addr))(note + 1) - start < size) {
		...
		if (note->n_namesz == 4 && note->n_type == NT_GNU_PROPERTY_TYPE_0 &&
			...
			do {
				...
				ptr += 8;
				if ((ptr + datasz) > ptr_end) return;

				last_type = type;

				/* Target specific property processing.  */
				if (_dl_process_gnu_property(l, fd, type, datasz, ptr) == 0) return;
...
static
#endif
struct link_map *_dl_map_object_from_fd(const char *name, const char *origname, int fd,
...
fail_new:
...

	{
		...
		for (ph = phdr; ph < &phdr[l->l_phnum]; ++ph) switch (ph->p_type) {
			...
			case PT_LOAD:
				...
				/* Determine whether there is a gap between the last segment
					 and this one.  */
				if (nloadcmds > 1 && c[-1].mapend != c->mapstart) has_holes = true;

				/* Optimize a common case.  */
#if (PF_R | PF_W | PF_X) == 7 && (PROT_READ | PROT_WRITE | PROT_EXEC) == 7
				c->prot =
					(PF_TO_PROT >> ((ph->p_flags & (PF_R | PF_W | PF_X)) * 4)) & 0xf;
#else
				c->prot = 0;
				if (ph->p_flags & PF_R) c->prot |= PROT_READ;
				if (ph->p_flags & PF_W) c->prot |= PROT_WRITE;
				if (ph->p_flags & PF_X) c->prot |= PROT_EXEC;
#endif
				break;

			case PT_TLS:
				if (ph->p_memsz == 0) /* Nothing to do for an empty segment.  */
					break;
		...
		for (size_t i = 0; i < nloadcmds; i++) loadcmds[i].mapalign = p_align_max;
	...
	if (l->l_ld != 0) l->l_ld = (ElfW(Dyn) *)((ElfW(Addr))l->l_ld + l->l_addr);
	...
	if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_FILES))
		_dl_debug_printf(
				...
				(int)sizeof(void *) * 2, maplength, (int)sizeof(void *) * 2,
				(unsigned long int)l->l_entry, (int)sizeof(void *) * 2,
				(unsigned long int)l->l_phdr, (int)sizeof(void *) * 2, l->l_phnum);
	...
	if (l->l_flags_1 & DF_1_INITFIRST) GL(dl_initfirst) = l;
	...
	if (__glibc_unlikely(origname != NULL)) add_name_to_object(l, origname);
#else
	...
	/* If we have newly loaded libc.so, update the namespace
		 description.  */
	if (GL(dl_ns)[nsid].libc_map == NULL && l->l_info[DT_SONAME] != NULL &&
	...
	if (r->r_state == RT_CONSISTENT) {
		...
		/* Auditing checkpoint: we are going to add new objects.  Since this
			 ...
			 to not be empty.  */
		if ((mode & __RTLD_AUDIT) == 0) _dl_audit_activity_nsid(nsid, LA_ACT_ADD);
#endif
	...
	if (!GL(dl_ns)[l->l_ns]._ns_loaded->l_auditing) _dl_audit_objopen(l, nsid);
#endif
...
//static int openat_verify(const char *name, int fd, struct filebuf *fbp,
...
#define VALID_ELF_ABIVERSION(osabi, ver) (ver == 0)
	...
	if (fd != -1) {
		...
		do {
			ssize_t retlen =
				__read_nocancel(fd, fbp->buf + fbp->len, sizeof(fbp->buf) - fbp->len);
			if (retlen <= 0) break;
			fbp->len += retlen;
		...
		if (__glibc_unlikely(fbp->len < (ssize_t)sizeof(ElfW(Ehdr)))) {
			errval = errno;
			errstring =
				(errval == 0 ? N_("file too short") : N_("cannot read file data"));
lose:
			if (free_name) {
			...
			if (ph->p_type == PT_NOTE && ph->p_filesz >= 32 &&
				...
				if (ph->p_offset + size <= (size_t)fbp->len)
					...
					if (size < __MAX_ALLOCA_CUTOFF)
						...
						if (abi_note_malloced == NULL) goto read_error;

						abi_note = abi_note_malloced;
					}
					if (__pread64_nocancel(fd, (void *)abi_note, size, ph->p_offset) !=
							size) {
				...
				while (memcmp(abi_note, &expected_note, sizeof(expected_note))) {
					ElfW(Addr) note_size =
						ELF_NOTE_NEXT_OFFSET(abi_note[0], abi_note[1], ph->p_align);
				...
				osversion = (abi_note[5] & 0xff) * 65536 + (abi_note[6] & 0xff) * 256 +
					(abi_note[7] & 0xff);
...
static int open_path(const char *name, size_t namelen, int mode,
	...
	do {
		...
		for (cnt = 0; fd == -1 && cnt < ncapstr; ++cnt) {
			...
			if (this_dir->status[cnt] == nonexisting) continue;
			...
			if (this_dir->status[cnt] == unknown) {
				if (fd != -1) this_dir->status[cnt] = existing;
...
struct link_map *_dl_map_object(struct link_map *loader, const char *name,
	...
	for (l = GL(dl_ns)[nsid]._ns_loaded; l; l = l->l_next) {
		/* If the requested name matches the soname of a loaded object,
			 ...
			 yet been opened.  */
		if (__glibc_unlikely((l->l_faked | l->l_removed) != 0)) continue;
	...
	if (__glibc_unlikely(GLRO(dl_naudit) > 0)) {
		...
		if (before != name && strcmp(before, name) != 0) origname = before;
	}
#endif
	...
	if (strchr(name, '/') == NULL) {
		...
		if (loader == NULL || loader->l_info[DT_RUNPATH] == NULL) {
			if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("name: %s\n", name);
			...
			for (l = loader; l; l = l->l_loader)
				if (cache_rpath(l, &l->l_rpath_dirs, DT_RPATH, "RPATH")) {
					...
					if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=1, fd:%d\n", fd);
					if (fd != -1) break;
			...
			if (fd == -1 && !did_main_map && main_map != NULL &&
					...
					cache_rpath(main_map, &main_map->l_rpath_dirs, DT_RPATH, "RPATH")){
				fd = open_path(name, namelen, mode, &main_map->l_rpath_dirs, &realname,
						&fb, loader ?: main_map, LA_SER_RUNPATH,
						&found_other_class);
				if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=2, fd:%d\n", fd);
			}
			/* Also try DT_RUNPATH in the executable for LD_AUDIT dlopen
				 call.  */
			if (__glibc_unlikely(mode & __RTLD_AUDIT) && fd == -1 && !did_main_map &&
					main_map != NULL && main_map->l_type != lt_loaded) {
				struct r_search_path_struct l_rpath_dirs;
				l_rpath_dirs.dirs = NULL;
				if (cache_rpath(main_map, &l_rpath_dirs, DT_RUNPATH, "RUNPATH")){
					fd =
						open_path(name, namelen, mode, &l_rpath_dirs, &realname, &fb,
								loader ?: main_map, LA_SER_RUNPATH, &found_other_class);
					if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=3, fd:%d\n", fd);
				}
			}
		}
		...
		if (fd == -1 && __rtld_env_path_list.dirs != (void *)-1){
			fd = open_path(name, namelen, mode, &__rtld_env_path_list, &realname, &fb,
					loader ?: GL(dl_ns)[LM_ID_BASE]._ns_loaded, LA_SER_LIBPATH,
					&found_other_class);

					if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=4, fd:%d\n", fd);
		}
		...
		if (fd == -1 && loader != NULL &&
			...
			fd = open_path(name, namelen, mode, &loader->l_runpath_dirs, &realname,
					...
					if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=5, fd:%d\n", fd);
		}
		...
		if (fd == -1) {
			...
			if (realname != NULL) {
				...
				if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=6, fd:%d\n", fd);
		...
		if (fd == -1 &&
			...
			fd = open_path(name, namelen, mode, &__rtld_search_dirs, &realname, &fb,
					l, LA_SER_DEFAULT, &found_other_class);

				if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("in dl_map_object: num=8, fd:%d\n", fd);
		}
		...
		if (realname == NULL)
			...
			if (GL(dl_exec_fd) > 0){
				if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_LIBS)) _dl_debug_printf("line 2059: GL(dl_exec_fd): %d\n", GL(dl_exec_fd));
...
no_file:
	...
	/* In case the LOADER information has only been provided to get to
		 ...
		 anymore.  */
	if (mode & __RTLD_CALLMAP) loader = NULL;
	...
	if (__glibc_unlikely(fd == -1)) {
		if (trace_mode &&
			...
			if ((name_copy = __strdup(name)) == NULL ||
					(l = _dl_new_object(name_copy, name, type, loader, mode, nsid)) ==
					NULL) {
				free(name_copy);
			...
			_dl_signal_error(errno, name, NULL, N_("cannot open shared object file"));
	}

	void *stack_end = __libc_stack_end;
	return _dl_map_object_from_fd(name, origname, fd, &fb, realname, loader, type,
			mode, &stack_end, nsid);
}
...
static void add_path(struct add_path_state *p,
		const struct r_search_path_struct *sps,
		unsigned int flags) {
	if (sps->dirs != (void *)-1) {
		struct r_search_path_elem **dirs = sps->dirs;
		do {
			...
			if (p->counting) {
				...
				if (r->dirnamelen < 2)
					*p->allocptr++ = r->dirnamelen ? '/' : '.';
					...
					p->allocptr = __mempcpy(p->allocptr, r->dirname, r->dirnamelen - 1);
				*p->allocptr++ = '\0';
				sp->dls_flags = flags;
			}
		} while (*dirs != NULL);
...
void _dl_rtld_di_serinfo(struct link_map *loader, Dl_serinfo *si,
		bool counting) {
	if (counting) {
		si->dls_cnt = 0;
		si->dls_size = 0;
